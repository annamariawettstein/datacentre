...
Method: union-find (connected components) over four linkage rules, applied only
...
def dedupe() -> tuple[int, int]:
    ...
    try:
        rows = conn.execute(
            """SELECT name, uid, area_name, postcode, address, associated_id
        ...
        for name, uid, area, postcode, address, associated_id in rows:
            if uid:
        ...
        # Rule 1 & 4: union within each shared postcode / address group.
        for group in list(by_postcode.values()) + list(by_address.values()):
